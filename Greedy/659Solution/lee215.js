...
const isPossible = (nums) => {
    ...
    for (const i of nums) {
        console.log(end[i - 1], left, end);
        if (left[i] == 0) continue;
        left[i]--;
        if (end[i - 1] > 0) {
            end[i - 1]--;
            if (end.hasOwnProperty(i)) {
                end[i]++;
            } else {
                end[i] = 1;
            }
...
const main = () => {
    ...
    // console.log(isPossible(nums)); // true
    // console.log(isPossible(nums2)); // true
    // console.log(isPossible(nums3)); // false
