...
// Accepted --- 104ms 65.38%
// reference: https://leetcode.com/problems/largest-divisible-subset/discuss/84013/Very-short-Java-DFS-solution-using-Memoization
let memo = new Map();
...
const dfs = (nums, i) => {
    ...
    let maxLenLst = [];
    let div = i == 0 ? 1 : nums[i - 1];
    for (let k = i; k < nums.length; k++) { // fix time write like this for (k = i; k < nums.length; k++) {
        if (nums[k] % div == 0) {
            let tmp = dfs(nums, k + 1);
            // let lst = tmp;
            let lst = [...tmp]; // fix
            lst.push(nums[k]);
            if (lst.length > maxLenLst.length) {
                maxLenLst = lst;
            }
        }
    }
    // console.log(maxLenLst);
    memo.set(i, maxLenLst);
    // console.log(memo);
    return maxLenLst;
};

// Accepted --- 96ms 92.31%
// reference: https://www.geeksforgeeks.org/largest-divisible-subset-array/
const largestDivisibleSubset_DP = (nums) => {
    let n = nums.length;
    if (n == 0) return [];
    nums.sort((a, b) => a - b);
    let dp = new Array(n).fill(1); // store count of divisors of all elements 
    let prev = new Array(n).fill(-1); // store previous divisor
    let maxIdx = 0; // store index of largest element in maximum size subset
    for (let i = 1; i < n; i++) { // In i'th iteration, we find length of chain ending with nums[i]
        for (j = 0; j < i; j++) { // Consider every smaller element as previous element
            if (nums[i] % nums[j] == 0) {
                if (dp[i] < dp[j] + 1) {
                    prev[i] = j;
                    dp[i] = dp[j] + 1;
        ...
        if (dp[i] > dp[maxIdx]) { // Update last index of largest subset if size of current subset is more. 
            maxIdx = i;
        }
    }
    // console.log(dp, prev);
