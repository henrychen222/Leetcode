...
// Accepted
const maximizeGreatness = (a) => {
    let tree = new SplayTree(), res = 0;
...
// TLE
const maximizeGreatness1 = (a) => {
    let m = new MultiSet(a), res = 0;
    for (const x of a) {
        let idx = m.upper_bound(x);
        // pr(x, m.show(), idx, m.get(idx))
        if (idx < m.size()) {
            m.remove(m.get(idx));
            res++;
        }
...
const main = () => {
    let a = [1, 3, 5, 2, 1, 3, 1];
    let a2 = [1, 2, 3, 4];
    pr(maximizeGreatness(a))
    pr(maximizeGreatness(a2))
};
