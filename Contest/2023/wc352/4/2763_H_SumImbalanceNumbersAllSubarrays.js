/*
 * 07/01/23 evening
 * https://leetcode.com/contest/weekly-contest-352/problems/sum-of-imbalance-numbers-of-all-subarrays/
 */
...
// Accepted
const sumImbalanceNumbers = (a) => {
    let n = a.length, res = 0;
    for (let i = 0; i < n; i++) {
        let tree = new SplayTree(), cnt = 0;
        tree.insert(a[i]);
        for (let j = i + 1; j < n; j++) {
            let x = a[j];
            if (!tree.has(x)) {
...
// Accepted
// reference: uwi https://leetcode.cn/circle/discuss/501Jzp/ (TsReaper)
const sumImbalanceNumbers1 = (a) => {
    let n = a.length, res = 0;
    for (let i = 0; i < n; i++) {
        let tree = new SplayTree(), cnt = -1;
        for (let j = i; j < n; j++) {
            let x = a[j];
            // pr(x, a.slice(i,j+1), tree.has(x), tree.has(x-1), tree.has(x+1), "cnt", cnt);
            if (!tree.has(x)) {
                tree.insert(x);
                cnt++;
                if (tree.has(x - 1)) cnt--; // 
...
//////////////////////////////////////////////////////////////////////////
// WA
function sumImbalanceNumbers1(nums) {
    const n = nums.length;
    let sum = 0;
    for (let i = 0; i < n - 1; i++) {
        let max = nums[i];
...
const main = () => {
    ...
    pr(sumImbalanceNumbers(debug1)) // 0
    pr(sumImbalanceNumbers(debug2)) // 2
};
