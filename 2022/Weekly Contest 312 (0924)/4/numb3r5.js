...
// Accepted --- 732ms
const numberOfGoodPaths2 = (a, edges) => {
    let n = a.length, g = initializeGraph(n), f = Array(n).fill(0), ds = new DJSet(n), res = 0, vis = Array(n).fill(false);
    packUG(g, edges);
    let d = new Map();
    for (let i = 0; i < n; i++) {
    ...
    for (const [, A] of d) {
        ...
        for (const cur of A) res += ++f[ds.find(cur)];
        for (const cur of A) f[ds.find(cur)]--;
    }
    return res;
};
...
const main = () => {
    let a = [1, 3, 2, 1, 3], edges = [[0, 1], [0, 2], [2, 3], [2, 4]];
    let a2 = [1, 1, 2, 2, 3], edges2 = [[0, 1], [1, 2], [2, 3], [2, 4]];
    pr(numberOfGoodPaths(a, edges))
    pr(numberOfGoodPaths(a2, edges2))
};
