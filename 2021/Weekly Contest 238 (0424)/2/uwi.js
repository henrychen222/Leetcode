...
// read: https://leetcode.com/problems/frequency-of-the-most-frequent-element/discuss/1175090/JavaC%2B%2BPython-Sliding-Window
// Accepted --- 216ms
const mx = Math.max;
...
const maxFrequency = (a, k) => {
    stin(a);
    let n = a.length;
    let m = used = res = 0; // m: left pointer  sliding window
    for (let i = 0; i < n; i++) {
        // pr(used, i, m)
        if (i > 0) {
            used += (i - m) * (a[i] - a[i - 1]);
