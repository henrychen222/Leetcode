...
const getAncestors = (n, edges) => {
    ...
    for (let target = 0; target < n; target++) {
        let visit = new Set(), res = new Set();
        for (let v = 0; v < n; v++) {
            if (visit.has(v)) continue;
            if (dfs(v, visit, target, res)) {
                res.add(v);
            }
        }
        // pr("node", target, res, '\n');
        ans.push([...res].sort((x, y) => x - y));
    }
    return ans;
};

// reference: https://stackoverflow.com/questions/65976519/how-to-get-ancestors-of-a-vertex-of-dag-in-precedence-order
const dfs = (cur, visit, target, res) => {
    // pr(cur, visit, target, res);
    // visit.add(cur);
    for (const child of g[cur]) {
        if (!visit.has(child)) {
            // pr("child", child)
            if (child == target) return true;
            if (dfs(child, visit, target, res)) {
                res.add(child);
                return true;
            }
        } else {
            if (res.has(child)) return true;
        }
    }
    return false;
};
