...
const getAncestors = (n, edges) => {
    ...
    for (let i = 0; i < n; i++) {
        ...
        while (q.length) {
            // pr("res", res);
            let cur = q.shift();
            if (cur != i) res[cur].push(i); // set ancestors for current node, ignore itself
...
const getAncestors2 = (n, edges) => {
    let g = initializeGraph(n), res = initializeGraphSet(n);
    packDG(g, edges);
    for (let i = 0; i < n; i++) {
        let q = [i], visit = new Set([i]);
        while (q.length) {
            ...
            for (const child of g[cur]) {
                if (visit.has(child)) continue;
                res[child].add(cur);
                for (const ancestor of res[cur]) res[child].add(ancestor);
                visit.add(child);
                q.push(child);
            }
        }
