...
const maxOperations = (nums, k) => {
    ...
    for (let i = 0; i < n; i++) {
        ...
        while (i < j && nums[i] + nums[j] > k) { // for each i let j stops at the largest index with sum(i, j) <= k
            j--;
        }
        if (i < j && nums[i] + nums[j] == k) { // get all sum condition, with i keep the same, find all j
            cnt++;
            j--;
        }
    }
...
// Accepted --- 180ms
const maxOperations_modidy = (nums, k) => {
    nums.sort((a, b) => a - b);
    let n = nums.length;
    let cnt = 0;
